...
namespace GISTech.GISTerrainLoader
{
    public class GISTerrainLoaderTerrainSmoother
    {
        ...
        public static void SmoothTerrainHeights(List<TerrainObject> terrains, float m_TerrainHeightSmoothFactor)
        {
            ...
            {
                heightmap = terrain.terrainData.GetHeights(0, 0, terrain.terrainData.heightmapWidth, terrain.terrainData.heightmapHeight);

                float[,] smoothData = terrain.terrainData.GetHeights(0, 0, terrain.terrainData.heightmapWidth, terrain.terrainData.heightmapHeight);
                float numRows = terrain.terrainData.heightmapWidth;
                float numCols = terrain.terrainData.heightmapHeight;
        ...
        public static void SmoothTerrainSurface(List<TerrainObject> terrains, int m_TerrainSurfaceSmoothFactor)
        {
            ...
            {
                float numRows = terrain.terrainData.heightmapWidth;
                float c = terrain.terrainData.heightmapHeight;

                var generatedHeightMap = terrain.terrainData.GetHeights(0, 0, terrain.terrainData.heightmapWidth, terrain.terrainData.heightmapHeight);

                var heights = smooth(generatedHeightMap, new Vector2(numRows, numRows));
        ...
        private static float[,] smooth(float[,] heightMap, Vector2 arraySize)
        {
