...
public class BEController : MonoBehaviour
{
    ...
    /// Virtual Joystick
    /// </summary>
    public BEJoystick beJoystick; // set on inspector
    ...
    //v1.1 -Changed FindTargetObject to retrieve all objects with BETargetObject component, more suitable name than use tag=="Player"
    /// <summary>
    ...
    void Awake()
    {
        ...
        if (name == "BEController")
        {
            ...
            // v1.2 -changed beInstructionStack referencing from fixed child index to compare object name, avoid hierarchy mistakes
    ...
    // v1.3 -Refactored to inprove performance
    void Update()
    {
    ...
    //v1.2 -singleEnabledProgrammingEnv moved outside #if UNITY_EDITOR due to error on build
    // Scene section
    [HideInInspector]
