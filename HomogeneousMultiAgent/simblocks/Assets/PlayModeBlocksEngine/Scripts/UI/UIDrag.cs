...
public class UIDrag : MonoBehaviour, IDragHandler, IEndDragHandler, IPointerClickHandler, IPointerDownHandler, IBeginDragHandler, IPointerUpHandler
{
    ...
    public void OnDrag(PointerEventData eventData)
    {
        if (transform.parent.tag == "mainCanvas")
        {
            if (newBlock != null)
            {
                ...
                // v1.2 -adjust correctly block anchor on instantiate New Block
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        // v1.3 -Bug fix: ghost block keeps visible if block is destroyed
    ...
    void Update()
    {
        if (isDragging == false && holding == true)
        {
            ...
            // v1.2 -reduced hold time to show "right click" menu from 1s to 0.6s
            if (holdCounter > 0.6f)
            {
